...
function printVarBinds( varbinds ) {
	...
	for( var varbindIndex = 0; varbindIndex < varbinds.length; ++varbindIndex ) {
		writeToBuffer( varbinds[ varbindIndex ].key + new Array( maxKeyLength - varbinds[ varbindIndex ].key.length + 1 ).join( " " ) + ": " + varbinds[ varbindIndex ].value + "\n" );
	}
}

function getAdditionalInfo( varbinds ) {
	return new Promise( function( resolve, reject ) {
		var varbindAddress = varbinds.filter( function( varbind ) {
			return varbind.key === "Address";
		} )[ 0 ];
		var varbindCommunity = varbinds.filter( function( varbind ) {
			return varbind.key === "Community";
		} )[ 0 ];
		var varbindInterface = varbinds.filter( function( varbind ) {
			return varbind.key === "Interfaces";
		} )[ 0 ];
		...
		session.get( [ OID_NIC_NAMES + "." + varbindInterface.value ], function( error, varbinds ) {
			if( error ) {
				return reject( error );
			}

			writeToBuffer( varbinds[ 0 ].value.toString() + "\n" );
